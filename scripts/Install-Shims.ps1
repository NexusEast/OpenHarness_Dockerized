...
#
# Sandbox-aware. The shims forward to the sandboxed container; cwd handling
# (auto-mount with [y/N] confirmation) lives in Common.psm1::Get-OhdExecArgs.
[CmdletBinding()]
param(
    [string]$Repo,
    [string]$Bin,
...
if (-not $Bin)  { $Bin  = $script:OhdShimBinDir }
...
if ($tplContent -notmatch 'OHD_SHIM_TEMPLATE_VERSION\s*=\s*2') {
    Stop-OhdDie "Shim template at $template is not v2 (sandbox-aware). Refusing to install."
}
...
# Profile snippet for shorthand functions.
$profileSnippet = Join-Path $script:OhdHome 'profile.ps1'
@"
# OpenHarness Sandbox - PowerShell integration
...
if ($AddToProfile) {
    if (-not $PROFILE) { Write-OhdWarn "No `\$PROFILE detected; cannot auto-edit." }
        ...
        if ($existing -notmatch [regex]::Escape($profileSnippet)) {
            ...
            Add-Content -Path $PROFILE -Value "`n# Added by OpenHarness Sandbox`n$line`n"
